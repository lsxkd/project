...
/*多天*/
var learningdays=[
    {lyear:2016,lmonth:10,lday:1,lno:2},
    {lyear:2016,lmonth:10,lday:15,lno:1},
    {lyear:2016,lmonth:10,lday:23,lno:3}
]
function setHolidayred(){
    ...
    for(var i=0;i<rows.length;i++){
        for(var j=0;j<rows[i].cells.length;j++){
            ...
            /*课程日期 样式*/
            for(var n=0;n<learningdays.length;n++){
                if(arr[0]==learningdays[n].lyear && arr[1]==learningdays[n].lmonth+1 && arr[2]==learningdays[n].lday){
                    var oSpan=document.createElement("span");
                    oSpan.className="Learningday"
                    oSpan.innerHTML=learningdays[n].lno+"节课";
...
function setTrHeight(){
    ...
    for(var i=0;i<rows.length;i++){
        rows[i].style.height=1.4+"rem";
        var tds=rows[i].getElementsByTagName("td");
        for(var j=0;j<tds.length;j++){
            var a=tds[j].childNodes[0];
            a.style.width=100+"%";
            a.style.height=1.4+"rem";
            a.style.lineHeight=1.4+"rem";
        }
    }
}
//得到id对象
