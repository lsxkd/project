...
function saveRedpacketBtn(obj){
	...
	var re = /^[\u4e00-\u9fa5a-z\d]*$/gi;//只能输入汉字和英文字母
		...
		$(".numberOnes",$(".classs_show")).each(function(){
			...
			if($(this).values() >= $(".numberTwos",$(".classs_show")).val()){
		...
		$(".limit_sizes",$(".classs_show")).each(function(){
			...
			if($(this).values() == "" || parseFloat($(".limit_sizes",$(".classs_show")).val()) == 0){
				$(this).addClass("info_borders_red");
			}
			if($(this).values() > 3000){
				$(this).addClass("info_borders_red");
		...
		$(".more_than_rero",$(".classs_show")).each(function(){
			if(parseFloat($(this).values()) == 0 || $(".more_than_rero",$(".classs_show")).val() == ""){
				$(this).addClass("info_borders_red");
			}
		})
