...
//弹出层调出js  start
	$(function(){
	//改变ie8 select右边三角  start
	$("select").after("<span></span>");
		$("select").each(function(){
			var w=$(this).css("width"),
				h=$(this).css("height");
			//console.log(w	);
			var spans=$(this).next("span");
			spans.append($(this));
			spans.css({
					...
					"background":"#FFF url(http://duchunyang.com/zhlt/1yiql/resources/images/arrow.png) no-repeat right center"
				})
				$(this).css({
					"width":"120%",
					//"line-height":"105%",
...
// 拖动DIV  end
	...
	$(function(){
		...
		//验证弹出框是否可以下一步  end		
		//验证弹出框是否可以关闭  start
			$(".info_verificatio_click,.info_verificatio_recharge_click,.verificatio_password_reset_click,.verificatio_account_click,.create_vip_code_click").click(function(){
				if($(this).closest(".Pop_layer_box").find("input").hasClass("info_border_red")){
