...
$(function(){
	...
	$(".bodys_id").on("click",".type_add_id",function(){
		if($(".items_id",$(".classs_show")).length>=5){
			alertErrors("红包活动类型不能超过5个", 2000);
			return false;
		}
	...
	//充值金额只能输入数字和小数点，小数点只能输入一个，小数点后只能输入两位 start
	 var keyCodes,   // 声明一个变量待用;
	  ...
	  $(".amount_of_charge").live("keydown",function(){
		 if(keydown==false) return false;   //阻止长按多次输入;
		 keydown=false;
		 keyCodes=$(this).val();   //提取当前值;
	  });
	  $(".amount_of_charge").live("keyup",function(e){
		keydown=true;
...
$(function(){
	$(".numberOnes,.numberTwos",$(".classs_show")).live("focus",function(){
		$(".numberOnes,.numberTwos",$(this).closest("div")).removeClass("info_borders_red");
	})

	$(".numbers_limits").live("keyup",function(){
		regxs=/^[1-9]+\d*$/ //声明大于等于一的正则;
		if(!regxs.test($(this).val())){ //若输入不是正确格式,本次输入无效
		  $(this).val("");
		}
	})
	$(".numbers_limits").live("blur",function(){
		regxs=/^[1-9]+\d*$/ //声明大于等于一的正则;
		if(!regxs.test($(this).val())){ //若输入不是正确格式,本次输入无效
		  $(this).val("");
		}
...
function saveRedpacketBtn(obj){
	...
	var re = /^[\u4e00-\u9fa5a-z\d]*$/gi;//只能输入汉字和英文字母
		...
		$(".numberOnes",$(".classs_show")).each(function(){
			if($(this).val() >= $(this).closest(".pop_item_types_con").nextAll(".pop_item_types_con").find(".numberTwos").values()){
				$(this).addClass("info_borders_red");
				$(this).closest(".pop_item_types_con").nextAll(".pop_item_types_con").find(".numberTwos").addClass("info_borders_red");
				alertErrors("赠送金额区间前面值必须小于后面值", 2000);
			}
		})
		$(".limit_sizes",$(".classs_show")).each(function(){
			if($(this).val() == "" || parseFloat($(".limit_sizes",$(".classs_show")).val()) == 0){
				$(this).addClass("info_borders_red");
				alertErrors("输入框不能为空或等于0", 2000);
			}
			if($(this).val() > 3000){
				$(this).addClass("info_borders_red");
				alertErrors("赠送金额不能超过3000", 2000);
			}
		})
		$(".more_than_rero",$(".classs_show")).each(function(){
			if(parseFloat($(this).val()) == 0 || $(".more_than_rero",$(".classs_show")).val() == ""){
				$(this).addClass("info_borders_red");
				alertErrors("输入框不能为空或等于0", 2000);
			}
		})
